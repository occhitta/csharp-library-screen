...
/// 警告画面情報クラスです。
/// </summary>
internal sealed class WarningScreenData : ElementScreenData {
	#region メンバー定数定義
	/// <summary>
	/// 表題名称
	/// </summary>
	private const string SourceName = "警告";
	/// <summary>
	/// 表示名称
	/// </summary>
	...
	private void ActionSelectMenu(object? dialogData, EventArgs optionData) {
		if (dialogData is WarningDialogData chooseData) {
	...
	protected override void ActionInvokeMenu() {
		...
		var dialogData = new WarningDialogData(headerText, detailData);
	...
	protected override void ActionCancelMenu() {
		if (DialogData is WarningDialogData dialogData) dialogData.SelectHook -= ActionSelectMenu;
		DialogData = null;
